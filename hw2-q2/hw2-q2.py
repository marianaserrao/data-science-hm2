...
class CNN(nn.Module):
    ...
    def __init__(self, dropout_prob):
        ...
        #mnist: (28,28,1)
        #padding_conv1=(f-1)/2=(5-1)/2=2
        self.conv1 = nn.Conv2d(1, 8, kernel_size = 5, stride = 1, padding = 2)
        self.relu1 = nn.ReLU()
        self.max_pool1 = nn.MaxPool2d(kernel_size = 2, stride = 2)
        #out_size_conv1 = 28
        #out_size_pool1 = 28/2 = 14       
        self.conv2 = nn.Conv2d(8, 16, kernel_size = 3, stride = 1, padding = 0)
        self.relu2 = nn.ReLU()
        self.max_pool2 = nn.MaxPool2d(kernel_size = 2, stride = 2)
        #out_size_conv2 = (n-f)/s+1= (14-3)/1+1 = 12
        #out_size_pool2 = 10/2 = 6
        self.affine1 = nn.Linear(in_features=16*6*6, out_features=600)
        self.relu3 = nn.ReLU()
        self.dropout = nn.Dropout(p=dropout_prob)        
        self.affine2 = nn.Linear(in_features=600, out_features=120)
        self.relu4 = nn.ReLU()        
        self.affine3 = nn.Linear(in_features=120, out_features=10)
        self.log_softmax = nn.LogSoftmax(dim=1)

        
    def forward(self, x):
        """
        x (n_examples x n_features): a batch of training 
        examples
